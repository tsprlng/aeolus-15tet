...
void Rankwave::gen_waves (Addsynth *D, float fsamp, float fbase, unsigned char scale_divisions, float *scale)
{
	...
	{
		_pipes [i - _n0].genwave (D, i - _n0, fsamp, ldexpf (fbase * scale [i % scale_divisions], i / scale_divisions - 5));
	}
	_modif = true;
}
